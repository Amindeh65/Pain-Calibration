function [trajectory, display_onset, RT, response_onset, biopac_display_onset, angle] = circular_rating_output1(duration, p, scale_tex, rating_type, channel, channel_type)
...
%%  edited Amin Dehghani 11/1/2021 

SAMPLERATE = .01; % used in continuous ratings
TRACKBALL_MULTIPLIER=1;

%% set default NAN values ______________________________________________________
% will be filled in with response if participant presses button
trajectory = NaN;
display_onset= NaN;
RT = NaN;
response_onset = NaN;
...
%%% configure screen ___________________________________________________________
...
dspl.cscale.width =0.9* p.ptb.rect(3); % image scale width
dspl.cscale.height = 0.45*p.ptb.rect(3); % image scale height
dspl.cscale.xcenter = 0.45*p.ptb.rect(3); % scale center (does not equal to screen center)
dspl.cscale.ycenter = 0.225*p.ptb.rect(4);
